...
pipeline {
 ...
 agent { 
    docker {
            label 'linux'
            image '352708296901.dkr.ecr.eu-central-1.amazonaws.com/alexey_jenk_agent:1'
        ...
        stage('Inputs') {
            steps {
                script {
                    ...
                    if (!params.Build_Type == "auto_trigger"){
                        println("===================================${STAGE_NAME}=============================================")
...
post {
    ...
    always {
        script {
            if(JOB.deploy == true){
                sh '$(docker images | grep amazonaws.com | tr -s ' ' | cut -d ' ' -f 3)'
            }
